...
%Atomic Percent
    ...
    atFe = (100-(atAl)-(atCr))./3;
    atNi = (100-(atAl)-(atCr))./3;
    atCu = (100-(atAl)-(atCr))./3;
    
    [atAl,atCr]=meshgrid(atAl,atCr);
...
format bank; %ini apa? format penulisan yang menggunakan 2 desimal dibelakang koma
    %Output 1-1
    %Result
    result = [atAl(:)'; atCr(:)'; Gchem(:)'; deltaGmg(:)'; deltaGtotal(:)'; SFE(:)']' ;
    title = '       atAl        atCr      Gchemical(J/mol)     Gmagnetic(J/mol)      deltaG(J/mol)           SFE(mJ/m2)';  
    disp(title), disp(result);

    figure(1);
    contourf(atAl,atCr,Gchem);
    xlabel('atAl'); ylabel('atCr'); zlabel('Gchemical (J/mol)');
    set(gca,'YDir','reverse');
        colorbar;
    hold on;
    figure(2);
    contourf(atAl,atCr,deltaGmg);
    xlabel('atAl'); ylabel('atCr'); zlabel('Gmagnetic (J/mol)');
    set(gca,'YDir','reverse');
        colorbar;
    hold on;
    figure(3);
    contourf(atAl,atCr,deltaGtotal)
    xlabel('atAl'); ylabel('atCr'); zlabel('deltaG(J/mol)');
    set(gca,'YDir','reverse');
        colorbar;
    hold on;
    figure(4);
    contourf(atAl,atCr,SFE)
    xlabel('at%-Al'); ylabel('at%-Cr'); zlabel('SFE(mJ/m2)');
    set(gca,'YDir');
        colorbar;
    hold on;
